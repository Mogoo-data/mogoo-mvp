...
export default function CaculatorPage() {
  ...
  function onSubmit(values: z.infer<typeof formSchema>) {
    const inputData: InputData = {
      ...
      electricity_type: Number(values.electricity_type),
      growth_rate: Number(values.growth_rate),
      industry: values.industry,
      procure_option: values.procure_option.map((option) => {
        const optionIndex = formOptions.procureOptions.findIndex((opt) => opt === option);
        return optionIndex !== -1 ? optionIndex : 0; 
      }),
      site_type: Number(values.site_type),
      annual_consumption: Number(values.annual_consumption),
      target_ratio: Number(values.target_ratio),
      target_year: Number(values.target_year),
      use_rec: false, // Assuming use_rec is not part of the form
      use_onsite: false, // Assuming use_onsite is not part of the form
      onsite_area: 0, // Assuming onsite_area is not part of the form
    };

    fetch("/api/histories/add", {
      method: "POST",
      headers: {
      ...
      if (data && data.id) {
        window.location.href = `/histories/${data.id}`;
      } else {
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <SelectComponent
                          ...
                          options={formOptions.countries.map(option => ({
                            label: option,
                            value: option
                          }))}
                        />
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <SelectComponent
                          ...
                          options={formOptions.industryOptions.map(option => ({
                            label: option,
                            value: option
                          }))}
                        />
              ...
              <Card>
                ...
                <CardContent className="space-y-6">
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <SelectComponent
                          ...
                          options={formOptions.electricityTypeOptions.map((option) => ({
                            label: option.label,
                            value: option.value
                          }))}
                        />
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <SelectComponent
                          ...
                          options={formOptions.targetYear.map((year) => ({ label: year, value: year }))}
                        />
                        <FormMessage />
                      </FormItem>
                    )}
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel htmlFor="expectedchangeRate">Growth Rate (%)</FormLabel>
                        <FormControl>
                          <Input id="expectedchangeRate" type="number" placeholder="10" {...field} />
                        </FormControl>
                        <FormMessage />
                        <p className="text-xs text-muted-foreground">
                          Estimated growth rate of your company's electricity consumption over time.
                        </p>
                      </FormItem>
                    )}
                  />
                ...
                <CardFooter className="flex justify-between">
                  <Button variant="outline" onClick={() => setFormStep(1)} className="hover:bg-gray-200 dark:hover:bg-gray-700">
                    Previous Step
                  </Button>
                  <Button
                    type="button"
